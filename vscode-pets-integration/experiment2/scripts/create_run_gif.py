#!/usr/bin/env python3
"""
Create running animation GIF from video
"""
...
VIDEO_PATH = "/home/ubuntu/forkMonkey/vscode-pets-integration/experiment2/videos/running_monkey.mp4"
FRAMES_DIR = "/home/ubuntu/forkMonkey/vscode-pets-integration/experiment2/frames_run"
...
def extract_frames_from_video(video_path, output_dir, fps=4):
    ...
    print(f"📹 Extracting frames from running video at {fps} FPS...")
    ...
    cmd = [
        ...
        '-i', video_path,
        '-vf', f'fps={fps}',
        os.path.join(output_dir, 'run_frame_%03d.png'),
        '-y'
    ]
    ...
    frame_files = sorted(glob.glob(os.path.join(output_dir, 'run_frame_*.png')))
...
def create_running_cycle(frame_files, output_path, cycle_length=8):
    """Create a running cycle GIF - running needs more frames than walking"""
    print(f"\n🏃 Creating running cycle GIF...")
...
def main():
    print("="*70)
    print("🏃 CREATING RUN ANIMATION FROM VIDEO")
    print("="*70)
    ...
    # Create running cycle GIF (8 frames for more dynamic motion)
    output_path = os.path.join(GIFS_DIR, "brown_run_8fps.gif")
    create_running_cycle(frame_files, output_path, cycle_length=8)
    ...
    exp1_run = "/home/ubuntu/forkMonkey/vscode-pets-integration/experiment1/assets/fork-monkey/brown_run_8fps.gif"
    if os.path.exists(exp1_run):
        exp1_size = os.path.getsize(exp1_run)
